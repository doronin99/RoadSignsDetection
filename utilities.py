...
def clean_data(annotation_folder, min_limit, max_limit):
    ...
    for file, classes in file_to_classes.items():
        delete_file = any(class_counts[class_id] < min_limit for class_id in classes) or \
                      any(class_counts[class_id] > max_limit for class_id in classes)
        if delete_file:
    ...
    for filename in os.listdir(annotation_folder):
        if filename.endswith('.txt'):
            ...
            for line in lines:
                ...
                if class_index in class_mapping and class_counts[class_index] < threshold:
